...
export const King = (i, j, canMoveTo, Board, turn) => {
  ...
  let rookKingSide = Board[i][j + 3]
  if (
    Board[i][j + 3] &&
    Board[i][j + 3].type === "Rook" &&
    // The king and rook involved in King Side castling must not have previously moved.
    king.numOfMoves === 0 &&
    rookKingSide.numOfMoves === 0 &&
  ...
  let rookQueenSide = Board[i][j - 4];
  if (
    Board[i][j - 4] &&
    Board[i][j - 4].type === "Rook" &&
    // The king and rook involved in Queen Side castling must not have previously moved.
    king.numOfMoves === 0 &&
    rookQueenSide.numOfMoves === 0 &&
    // There must be no pieces between the king and the Queen Side rook.
    Board[i][j - 1] === null &&
    Board[i][j - 2] === null &&
    Board[i][j - 3] === null
  ) {
    // The king may not currently be in check, nor may the king pass through or end up in a square that is under:
    // Attack by an enemy piece.
    // Checking whether the king would be under check if queenside castling did happen.
    const board = Board.map(inner => inner.slice());
...
export const Pawn = (i, j, canMoveTo, Board, turn) => {
  ...
  if (turn === "W" && i !== 0) {
    ...
    if (Board[i - 1][j] === null) {
      ...
      newBoard[i][j] = null
      if (!isUnderCheck(newBoard, turn === "W" ? "B" : "W"))
        // Highlighting the box above the pawn.
        canMoveTo[i - 1][j] = true
      if (
        Board[i][j] &&
    ...
    for (let j = 0; j < 8; j++) {
      if (board[i][j] && board[i][j].color === checkForWhom) {
        ...
        switch (board[i][j].type) {
          ...
          case "Queen":
            isGivingCheck = RookGivesCheck(i, j, board)
            if (!isGivingCheck) isGivingCheck = BishopGivesCheck(i, j, board)
            break
          case "King":
...
export const stalemate = (board, turn) => {
  // Check for lack of legal moves.
  ...
  for (let i = 0; i < 8; i++) {
    for (let j = 0; j < 8; j++) {
      ...
      if (piece && piece.color === turn) {
        ...
        // Check if there are any legal moves for this piece.
        if (piece.canMoveTo.some(row => row.includes(true))) {
          // Found a legal move, not stalemate.
          noLegalMoves = false;
        }
        // Check if the player has sufficient material.
        if (
          // Exclude the king.
          piece.type !== "King" &&
          // Check if the piece has importance.
          piece.importance > 0
        ) {
  ...
  // If no legal moves were found for any piece, it's a stalemate.
  if (noLegalMoves) return true;

  // If there is insufficient material, it's a stalemate.
  if (insufficientMaterial) return true;

  // Otherwise, it's not a stalemate.
  return false;
};
